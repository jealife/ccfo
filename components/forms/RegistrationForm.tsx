      ...
      <div className="sports-card p-8 bg-card/40 backdrop-blur-xl border-white/5 min-h-[400px]">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentStep === 4 && <DocumentsStep data={formData.documents} updateData={(val: any) => setFormData({...formData, documents: {...formData.documents, ...val}})} teamName={formData.team.name} />}
            {currentStep === 5 && <PaymentStep data={formData.team} updateData={(val: any) => setFormData({...formData, documents: {...formData.documents, ...val}})} receiptData={formData.documents.receipt} />}
          </motion.div>
        </AnimatePresence>
...
function TeamStep({ data, updateData }: any) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <FormInput label="Nom de l'équipe" placeholder="ex: Village Bissobinam" value={data.name} onChange={(e: any) => updateData({name: e.target.value})} />
        <FormInput label="Village / Quartier d'origine" placeholder="ex: Okano Central" value={data.village} onChange={(e: any) => updateData({village: e.target.value})} />
        <FormInput label="Couleur du maillot principal" placeholder="ex: Vert et Jaune" value={data.color} onChange={(e: any) => updateData({color: e.target.value})} />
        <FormInput label="Président / Représentant" placeholder="Ondo Samuel" value={data.president} onChange={(e: any) => updateData({president: e.target.value})} />
        <FormInput label="Téléphone" placeholder="+241 ..." value={data.phone} onChange={(e: any) => updateData({phone: e.target.value})} />
        <FormInput label="WhatsApp" placeholder="+241 ..." value={data.whatsapp} onChange={(e: any) => updateData({whatsapp: e.target.value})} />
      </div>
    </div>
  );
}
          ...
          <div key={i} className="p-4 rounded-xl border border-border bg-white/5 flex gap-4 items-end">
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <FormInput label={`Nom Membre ${i + 1}`} placeholder="Nom complet" value={member.name} onChange={(e: any) => updateData(i, {name: e.target.value})} />
                <FormInput label="Rôle" placeholder="Coach, Assistant, etc." value={member.role} onChange={(e: any) => updateData(i, {role: e.target.value})} />
                <FormInput label="Origine" placeholder="Village" value={member.origin} onChange={(e: any) => updateData(i, {origin: e.target.value})} />
              </div>
            </div>
          </div>
        ))}
        ...
        {data.map((player: any, i: number) => (
          <div key={i} className="p-4 rounded-xl border border-border bg-white/5 group hover:border-primary/50 transition-colors">
            <div className="flex items-center gap-3 mb-4">
              ...
              <FormInput label="Nom Complet" className="flex-1" value={player.name} onChange={(e: any) => updateData(i, {name: e.target.value})} />
            </div>
...
function DocumentsStep({ data, updateData, teamName }: any) {
  const supabase = createClient();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        <FileUpload 
          ...
          onChange={(e: any) => handleUpload(e, 'idCards')}
        />
        <FileUpload 
          ...
          onChange={(e: any) => handleUpload(e, 'certificate')}
        />
      </div>
    </div>
  );
...
function PaymentStep({ data, updateData, receiptData }: any) {
  const supabase = createClient();
...
function FormInput({ label, placeholder, className, value, onChange }: any) {
  return (
...
function FileUpload({ label, description, value, isUploading, onChange }: any) {
  return (
